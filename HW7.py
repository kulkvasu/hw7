...
def make_players_table(data, cur, conn):
    ...
    for player in data['squad']:
        id = player['id']
        name = player['name']
        positions = posDict[player['position']]
        birthYear = player['dateOfBirth'][0:4]
        nationality = player['nationality']
        cur.execute("INSERT or IGNORE INTO Players (id, name, position_id, birthyear, nationality) VALUES (?,?,?,?,?)", (id, name, positions, birthYear, nationality))
